...
class OpenAIBaseResponsesClient(OpenAIBase, BaseChatClient):
        ...
        try:
            if not chat_options.response_format:
                ...
                chat_options.conversation_id = (
                    response.id if chat_options.store is True else None
                )
                return self._create_response_content(
                    response, chat_options=chat_options
                )
            ...
            parsed_response: ParsedResponse[
                BaseModel
            ] = await self.client.responses.parse(
                text_format=resp_format,
                stream=False,
                **options_dict,
            )
            chat_options.conversation_id = (
                parsed_response.id if chat_options.store is True else None
            )
            return self._create_response_content(
                parsed_response, chat_options=chat_options
            )
    ...
    def _prepare_chat_messages_for_request(
        ...
        list_of_list = [
            self._openai_chat_message_parser(message, call_id_to_id)
            for message in chat_messages
        ]
        ...
        for content in message.contents:
            match content:
                ...
                case FunctionResultContent():
                    ...
                    new_args.update(
                        self._openai_content_parser(
                            message.role, content, call_id_to_id
                        )
                    )
                    all_messages.append(new_args)
                case FunctionCallContent():
                    function_call = self._openai_content_parser(
                        message.role, content, call_id_to_id
                    )
            ...
            case TextReasoningContent():
                ...
                if content.additional_properties is not None:
                    if status := content.additional_properties.get("status"):
                        ret["status"] = status
                    if reasoning_text := content.additional_properties.get(
                        "reasoning_text"
                    ):
                        ret["content"] = {
                            "type": "reasoning_text",
    ...
    def _create_response_content(
                    ...
                    if hasattr(item, "outputs") and item.outputs:
                        for code_output in item.outputs:
                            if code_output.type == "logs":
                                contents.append(
                                    TextContent(
                                        text=code_output.logs, raw_representation=item
                                    )
                                )
                        ...
                        contents.append(
                            TextContent(text=item.code, raw_representation=item)
                        )
                        ...
                        if not uri.startswith("data:"):
                            ...
                            try:
                                ...
                                decoded_data = base64.b64decode(
                                    uri[:100]
                                )  # First ~75 bytes should be enough
                                if decoded_data.startswith(b"\x89PNG"):
                                    format_type = "png"
                                elif decoded_data.startswith(b"\xff\xd8\xff"):
                                    format_type = "jpeg"
                                elif (
                                    decoded_data.startswith(b"RIFF")
                                    and b"WEBP" in decoded_data[:12]
                                ):
                                    format_type = "webp"
                                elif decoded_data.startswith(
                                    b"GIF87a"
                                ) or decoded_data.startswith(b"GIF89a"):
                                    format_type = "gif"
                                else:
        ...
        args: dict[str, Any] = {
            ...
            "created_at": datetime.fromtimestamp(response.created_at).strftime(
                "%Y-%m-%dT%H:%M:%S.%fZ"
            ),
        ...
        if response.usage and (
            usage_details := self._usage_details_from_openai(response.usage)
        ):
    ...
    def _create_streaming_response_content(
        ...
        match event.type:
            ...
            case "response.completed":
                conversation_id = (
                    event.response.id if chat_options.store is True else None
                )
                ...
                match event_item.type:
                    ...
                    case "code_interpreter_call":  # ResponseOutputCodeInterpreterCall
                        if hasattr(event_item, "outputs") and event_item.outputs:
                            ...
                            contents.append(
                                TextContent(
                                    text=event_item.code, raw_representation=event_item
                                )
                            )
                    case "reasoning":  # ResponseOutputReasoning
...
TOpenAIResponsesClient = TypeVar(
    "TOpenAIResponsesClient", bound="OpenAIResponsesClient"
)
