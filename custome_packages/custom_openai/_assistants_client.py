...
class OpenAIAssistantsClient(OpenAIConfigMixin, BaseChatClient):
    ...
    async def _inner_get_streaming_response(
        ...
        **kwargs: Any,
        ...
        run_options, tool_results = self._prepare_options(
    ...
    async def _get_assistant_id_or_create(self) -> str:
        ...
        if self.assistant_id is None:
            created_assistant = await self.client.beta.assistants.create(
                name=self.assistant_name, model=self.model_id
            )
    ...
    async def _create_assistant_stream(
        ...
        tool_run_id, tool_outputs = self._convert_function_results_to_tool_output(
            tool_results
        )
    ...
    async def _get_active_thread_run(self, thread_id: str | None) -> Run | None:
        ...
        async for run in self.client.beta.threads.runs.list(
        ...
        **kwargs: Any,
        ...
        if chat_options is not None:
            ...
            if chat_options.allow_multiple_tool_calls is not None:
                run_options["parallel_tool_calls"] = (
                    chat_options.allow_multiple_tool_calls
                )
            ...
            if chat_options.tool_choice is not None:
                ...
                if (
                    ...
                    and chat_options.tools is not None
                ):
                ...
                if (
                    chat_options.tool_choice == "none"
                    or chat_options.tool_choice == "auto"
                ):
