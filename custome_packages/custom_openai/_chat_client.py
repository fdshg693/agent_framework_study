...
class OpenAIBaseChatClient(OpenAIBase, BaseChatClient):
        ...
        **kwargs: Any,
        ...
        try:
            return self._create_chat_response(
                await self.client.chat.completions.create(stream=False, **options_dict),
                chat_options,
            )
        ...
        for tool in tools:
            if isinstance(tool, ToolProtocol):
                match tool:
                    ...
                    case _:
                        logger.debug(
                            "Unsupported tool passed (type: %s), ignoring", type(tool)
                        )
            else:
        ...
        for tool in tools:
            if isinstance(tool, HostedWebSearchTool):
                ...
                return (
                    ...
                    and "user_location" in tool.additional_properties
                    else {}
                )
    ...
    def _prepare_options(
        ...
        if (
            ...
            options_dict["response_format"] = type_to_response_format_param(
                chat_options.response_format
            )
    ...
    def _create_chat_response(
        self, response: ChatCompletion, chat_options: ChatOptions
    ) -> "ChatResponse":
        ...
        for choice in response.choices:
            ...
            if parsed_tool_calls := [
                tool for tool in self._get_tool_calls_from_chat_choice(choice)
            ]:
                contents.extend(parsed_tool_calls)
            messages.append(ChatMessage(role="assistant", contents=contents))
        return ChatResponse(
            response_id=response.id,
            created_at=datetime.fromtimestamp(response.created).strftime(
                "%Y-%m-%dT%H:%M:%S.%fZ"
            ),
    ...
    def _create_chat_response_update(
        ...
        if chunk.usage:
            return ChatResponseUpdate(
                ...
                contents=[
                    UsageContent(
                        details=self._usage_details_from_openai(chunk.usage),
        ...
        for choice in chunk.choices:
            ...
            if text_content := self._parse_text_from_choice(choice):
                contents.append(text_content)
        return ChatResponseUpdate(
            created_at=datetime.fromtimestamp(chunk.created).strftime(
                "%Y-%m-%dT%H:%M:%S.%fZ"
            ),
            contents=contents,
            role=Role.ASSISTANT,
    ...
    def _get_tool_calls_from_chat_choice(
        ...
        if content and content.tool_calls:
            for tool in content.tool_calls:
                if (
                    not isinstance(tool, ChatCompletionMessageCustomToolCall)
                    and tool.function
                ):
    ...
    def _prepare_chat_history_for_request(
        ...
        """
        list_of_list = [
            self._openai_chat_message_parser(message) for message in chat_messages
        ]
    ...
    def _openai_chat_message_parser(self, message: ChatMessage) -> list[dict[str, Any]]:
        ...
        for content in message.contents:
            ...
            if isinstance(
                content,
                (FunctionApprovalRequestContent, FunctionApprovalResponseContent),
            ):
                continue
            ...
            match content:
                case FunctionCallContent():
                    if all_messages and "tool_calls" in all_messages[-1]:
                        ...
                        all_messages[-1]["tool_calls"].append(
                            self._openai_content_parser(content)
                        )
                    else:
